...
def add_to_cart(request):
    ...
    if user.is_anonymous():
        ...
        password = '1234567a'
        user = User.objects.create(username=user_name, first_name='guest', last_name='guest', email='guest@gmail.com',
...
def cart(request):
    ...
    return render(request, 'cart.html',
                  {'user': user, 'items': items, 'page': 'cart', 'cart_items': cart_items, 'sum': items_sum})
...
def confirm_order(request):
    ...
    return render(request, 'thankyou.html',
                  {'user': user, 'items': 0, 'page': 'cart', 'cart_items': cart_items, 'sum': items_sum,
